...
{
	mDefaultColors.push_back(Vector3(0.0f, 0.0f, 1.0f));
	mDefaultColors.push_back(Vector3(0.0f, 1.0f, 0.0f));
}
...
void ScoreBoardManager::AddEntry(uint32_t inPlayerId, const string& inPlayerName)
{
	...
	float r = glm::linearRand(0.4f, 1.0f);
	float g = glm::linearRand(0.4f, 1.0f);
	float b = glm::linearRand(0.4f, 1.0f);
	//mEntries.emplace_back(inPlayerId, inPlayerName, mDefaultColors[inPlayerId % mDefaultColors.size()]);
	mEntries.emplace_back(inPlayerId, inPlayerName, Vector3(r,g,b));
}
...
void ScoreBoardManager::SortEntriesByScore()
{
	std::sort(mEntries.begin(), mEntries.end(), [](Entry lhs, Entry rhs) {
